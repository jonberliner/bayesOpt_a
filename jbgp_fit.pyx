from jb.jbgp_1d import K_se, conditioned_mu, conditioned_covmat, sample
...
cpdef ndarray[double, ndim=1] get_trialError(ndarray[double, ndim=1] xObs,
    ...
    xerr = abs(hat[0] - xDrill)
    yerr = abs(hat[1] - yDrill)
...
cpdef ndarray[double, ndim=2] get_experimentError(df, lenscale, X):
    ...
    itrials = unique(df.itrial)
    # cdef ndarray[int, ndim=1] rounds = unique(df.round)
    cdef ndarray[double, ndim=2] trialError = zeros([len(itrials), 4])
    cdef int r
    for r in itrials:
        dfr = df[df.itrial==r]
        trialError[r-1] = get_trialError(npa(dfr.xObs.iat[0]),
                                         npa(dfr.yObs.iat[0]),
                                         npa(dfr.xDrill.iat[0]),
                                         npa(dfr.yDrill.iat[0]),
                                         X,
                                         KX,
                                         lenscale)
    return trialError
